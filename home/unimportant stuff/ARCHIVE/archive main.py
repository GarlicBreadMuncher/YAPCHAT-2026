...
def accept_chat(requester):
    ...
    #Deletes the request row from the database and then redirects to the private messaging page with that user.
...
def messaging(partner):
                            ...
                            #Redirects to browse if <partner> does not exist.
...
def handle_message(data):
    ...
    #SocketIO event: fired when a user sends a message. Saves the message permanently to the database then emits it back
    #-to everyone in the room (including the sender) so all open chat windows update in real time without a page refresh.

    #data = { 'room': '<room_id>', 'sender': '<username>', 'text': '<message>' }
    
    room   = data['room']
    sender = data['sender']
    text   = data['text']

    #Sends the message to the database so it survives even when the server restarts
    with get_db() as conn:
        conn.execute(
            'INSERT INTO messages (room, sender, text) VALUES (?, ?, ?)',
            (room, sender, text)
        )

    #Broadcast the message to all users currently in this room
